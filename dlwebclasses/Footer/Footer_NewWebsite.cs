...
namespace dlwebclasses
{
    public class Footer_NewWebsite:AFooter
    {
        ...
        public override StringBuilder getfooter(AContents _contents, Website_Pages webpage = null)
        {
            ...
            else
            {
                ...
                List<string> offInLondon = _off.getofficesInLondonList(webpage);
                List<string> offOutLondon = _off.getofficesOutLondonList(webpage);

                List<string> AreasofLaws = new List<string>();
                IT_DatabaseEntities db = new IT_DatabaseEntities();
                //List<string> DepartmenttoInclude = new List<string>(new string[]{ "Action Against Public Authorities", "Business Immigration", "Child Care", "Civil Liberties and Human Rights", "Civil Litigation", "Clinical Negligence", "Community Care", "Court of Protection", "Crime", "Employment", "Family", "Housing", "Immigration", "Personal Injury", "Prison Law", "Public Law", "Welfare Benefits", "Wills and Probate" });
                //AreasofLaws = db.Website_Department_Structure.Where(x => x.departmenttype == "AreaOfLaw" && DepartmenttoInclude.Contains(x.Name)).Select(x => "<li><a href=\"/" + x.Overview1 + "\">" + x.Name + "</a></li>").ToList();
                ...
                foreach (var item in offOutLondon)
                {
                    SB.AppendLine(item);
                    i++;
                    if (i == (int)(offOutLondon.Count/2) + 1)
                    {
                        SB.AppendLine("                            </ul>");
                        SB.AppendLine("                        </div>");
