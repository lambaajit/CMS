        ...
        public CostLaw_TeamPages(AContents _Acontent)
        {
            ...
            for (int j = 1; j <= 7; j++)
            {
                ...
                foreach (var emp in employeelist.Where(x => x.Profile_website == true))
                {
                    ...
                    SB.AppendLine("                                <span class=\"staffname\">" + emp.forename + " " + emp.surname + " </span><span class=\"jobtitle\">" + (emp.forename == "Yllka Olly" ? "Solicitor/Costs Manager" : emp.jobtitle.Replace("Manager and Supervisor","Manager/Supervisor")) + "</span>");
                    SB.AppendLine("                            </div>");
                    SB.AppendLine("                        </div>");
                    SB.AppendLine("                    </div>");
