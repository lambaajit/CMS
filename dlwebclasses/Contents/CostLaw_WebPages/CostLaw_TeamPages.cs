        ...
        public CostLaw_TeamPages(AContents _Acontent)
        {
            ...
            for (int j = 1; j <= 7; j++)
            {
                ...
                if (j == 1)
                    ...
                    employeelist = dbhr.Emp_Details.Where(x => x.employed == "1" && x.Profile_website == true && x.start_date < DateTime.Now && excludednames.Contains(x.forename + " " + x.surname) == false && (x.end_date == null || x.end_date > DateTime.Now || x.end_date.Value.ToString().Contains("1900")) && (x.jobtitle.Contains("Junior") == true || x.jobtitle.Contains("Trainee") == true) && (x.company_name == "Cost Law Services Limited" || x.company_name == "Cost Law Limited") && !x.jobtitle.Contains("Controlled Work") && !x.jobtitle.Contains("exceptional") && !x.jobtitle.Contains("Costs Lawyer")).OrderBy(y => y.forename).ToList();
                else if (j == 7)
